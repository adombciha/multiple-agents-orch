...
- 需要 Python 3 和 Python 包 `requests`；基本验证还需要 `pytest`。
- 使用默认 Git worktree 时需要 Git；将 `use_worktree` 设为 `false` 可停用。
- 请为每个启用的后端安装并登录对应服务：Ollama 服务器，或 `codex`、`agy`、`claude`、`grok` CLI。未运行其 AI 工作流程时无需安装所有 CLI。

从 Git 获取源码后，在项目根目录执行：
...
最后一行只验证 CLI 加载，不会调用 AI 或创建 worktree。

---

## 配置文件

`init` 会创建 `.ai-company/config.json`；未指定的键由 `orchestrator/core/config.py` 中的默认设置补齐。

| 设置键 | 用途 |
| --- | --- |
| `ollama_url`、`ollama_model` | Ollama 服务地址与默认模型。 |
| `test_command`、`max_revisions` | QA 执行的测试命令与计划／代码修订上限。 |
| `backends` | 每个角色使用的后端；`set-backend` 可更新支持的角色。 |
| `model_tiers`、`role_models`、`role_model_backends`、`role_model_tiers` | 后端模型列表、角色模型与模型路由。 |
| `use_ponytail`、`use_worktree` | 启用极简提示或 Git worktree 隔离。 |
| `backend_escalation_path`、`staffing_limits` | 后端升级顺序与 RD／QA 人力上限。 |
...
| Grok | RA 与 Sales 专家需要时的领域分析 | Grok CLI `grok-4.5` |
...
`review` 仅适用于 `WAITING_FOR_OWNER`：`pass` 按记录的核准路径继续、`revise` 创建修订任务并回到 `IMPLEMENTING`、`reject` 转为 `FAILED`；`--run` 只会在 `pass` 或 `revise` 后继续执行。
...
`run`、`step`、`approve --run` 可能调用外部 AI CLI、修改 Git worktree 或产生费用；未配置外部服务时，请先使用 `--help`、`verify_alignment.py` 与上述测试进行安全验证。
...
3. **多语言文档**：提供 `en`、`zh-TW`、`ja` 和 `zh-CN` README。
